...
<div class="fieldcontain ${hasErrors(bean: genOperationRecordInstance, field: 'operation', 'error')} required">
    <label for="operation">
        <g:message code="genOperationRecord.operation.label" default="Диагноз" />
        <span class="required-indicator">*</span>
    </label>
    <g:select id="operation" noSelection="['':'Выберите диагноз']" name="operation.id" from="${ru.mm2.operations.Operations.list()}" optionKey="id" required="" value="${genOperationRecordInstance?.operation?.id}" optionValue="name" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: genOperationRecordInstance, field: 'date_time', 'error')} required">
    ...
    <g:else>
        <g:datePicker name="date_time" precision="day"  value="${genOperationRecordInstance?.date_time}" relativeYears="[0..10]" />
    </g:else>
</div>
...
<div class="fieldcontain ${hasErrors(bean: genOperationRecordInstance, field: 'updatedUser', 'error')} ">
