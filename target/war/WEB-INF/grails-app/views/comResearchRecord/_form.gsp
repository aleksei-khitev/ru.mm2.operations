...
<div class="fieldcontain ${hasErrors(bean: comResearchRecordInstance, field: 'research', 'error')} required">
	...
	<g:select id="research" name="research.id" from="${ru.mm2.operations.Research.list()}" optionKey="id" required="" optionValue="name" value="${comResearchRecordInstance?.research?.id}" class="many-to-one"/>
</div>
